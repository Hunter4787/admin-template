...
const workerReducer=(state=workers,action)=>
{
  switch(action.type)
  {
      case 'ADD_worker' :
      return (
          state.concat(action.newworker)
      )
      case 'BLOCK_worker':
      return (
          state.map(el=>el._id===action.blockworker._id? el=action.blockworker:el)
      )
      case 'UNBLOCK_worker':
      return (
        state.map(el=>el._id===action.blockworker._id? el=action.blockworker:el)
