...
class CategoryList extends Component {
	...
	componentWillMount = () => {
		...
		if (this.props.souscategories.filter(el=>el.id_categorie.indexOf(this.props._id)> -1).length!==0)
		this.setState({
				Category: this.props.souscategories.filter(el=>el.id_categorie===this.props._id)
			});
	};
	...
	render() {
		...
		console.log("hello "+ this.props._id);
		const customStyles = {
			content: {
				top: '50%',
				left: '50%',
		...
		return (
			<div className="container category-list-container">
				...
				/>
				{this.state.Category.filter(el => {return (el.titre.toLowerCase().indexOf(this.state.title.toLowerCase()) > -1)
				}).map((el, index) => (
